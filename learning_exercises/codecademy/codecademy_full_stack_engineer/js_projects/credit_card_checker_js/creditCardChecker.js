...
function idInvalidCardCompanies(nestedArray) {
  ...
  nestedArray.forEach(arrayElement => {
    ...
    if (companies.includes(company) && !newArray.includes(company)) {
...
function convertToValid(array) {
  ...
  let sum = sumArray.reduce((accumulator, currentValue) => accumulator + currentValue);
  let diff = sum % 10;
  if (array[array.length - 3] >= diff) {
    array[array.length - 3] -= diff;
  } else {
    array[array.length - 3] += (10 - diff)
  }
  return array;
}
